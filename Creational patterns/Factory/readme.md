...
## ⚡ В двух словах
Объект создаётся не напрямую, а через фабричный метод, который можно переопределять в подклассах.

## 🧩 Пример из Design Patterns for Humans
_Пример с наймом: менеджер сам решает, какого интервьюера создать._
