## 🧩 Паттерн State на PHP  
_Состояние_

Отвечает за ситуацию, когда у объекта есть несколько состояний и он меняет поведение в зависимости от них.
...
- Когда телефон разблокирован, нажатие кнопок приводит к действиям.
...
## ⚡ В двух словах
Поведение объекта меняется вместе с его состоянием.

## 🧭 Когда использовать
- Поведение сильно зависит от внутреннего состояния.
- Большие `if/else` или `switch` начинают разрастаться.
- Нужно явное разделение состояний по классам.

## 💡 Ключевые идеи
- Состояния — отдельные классы.
- Контекст делегирует поведение текущему состоянию.
- Переходы между состояниями управляемы.
...
_Телефон меняет поведение в зависимости от состояния (заблокирован/разблокирован)._
