...
## 🎯 Применимость
- **Нужно выполнить операцию над всеми элементами сложной структуры.** Одна операция применяется к объектам разных классов.
- **Нужно добавить несвязанные операции, не засоряя классы.** Операции выносятся в посетителя.
- **Новое поведение нужно не всем классам иерархии.** Посетитель определяет его точечно.
...
## ⚡ В двух словах
Добавляем новые операции без изменения классов объектов.
...
- ✅ Реализуйте конкретные классы, которые будут принимать посетителей:
```php
<?php
...
_Доктор «посещает» животных и выполняет операции._
