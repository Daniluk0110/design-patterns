# 🔗 Паттерн Chain of Responsibility на PHP  
_CoR, Chain of Command, Chain of Responsibility, цепочка обязанностей_
...
## ⚡ В двух словах
Запрос проходит по цепочке обработчиков, пока один не сможет его обработать.

## 🧭 Когда использовать
- Нужно **разделить ответственность** между несколькими обработчиками.
- Не хочется жёстко связывать отправителя с конкретным получателем.
- Требуется **гибко менять** порядок обработчиков на лету.

## 💡 Ключевые идеи
- Каждый обработчик либо **обрабатывает**, либо **передаёт дальше**.
- Цепочка может быть динамически собрана.
- Клиенту не важно, кто именно обработал запрос.
...
_Платёж цепочкой из нескольких аккаунтов, пока не найдётся подходящий._

```php
<?php

$paymentA = new Account(100);
$paymentB = new Account(300);
$paymentC = new Account(1000);
