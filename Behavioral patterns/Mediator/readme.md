## 🤝 Паттерн Mediator на PHP  
_Intermediary, Controller, Mediator, Посредник_
...
## 🎯 Применимость
- **Сложно менять классы из-за множества хаотичных связей.** Посредник переносит связи в одно место и упрощает рефакторинг.
- **Класс нельзя переиспользовать из-за зависимостей.** После внедрения посредника компоненты общаются только через него.
- **Нужно много подклассов для разных контекстов.** Достаточно переопределить связи в посреднике.
...
### 3) Компоненты `Component1` и `Component2`
```php
<?php
...
_Чат-комната, через которую общаются пользователи._
