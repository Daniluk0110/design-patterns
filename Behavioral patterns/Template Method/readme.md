## 🧱 Паттерн Template Method на PHP  
_Шаблонный метод_
...
## 🎯 Применимость
- **Нужно расширять базовый алгоритм, не меняя его структуру.** Подклассы переопределяют отдельные шаги.
- **Есть несколько похожих классов с небольшими отличиями.** Общий алгоритм выносится в суперкласс, различия — в подклассы.
...
## ⚡ В двух словах
Есть «скелет» алгоритма, а подклассы переопределяют отдельные шаги.

## 🧭 Когда использовать
- Есть общий алгоритм с разными вариациями шагов.
- Хотите избежать дублирования кода в похожих классах.
- Нужен контроль над порядком выполнения шагов.

## 💡 Ключевые идеи
- Базовый класс определяет шаблон.
- Подклассы переопределяют только нужные шаги.
- Структура алгоритма остаётся неизменной.
...
_Сборка инструментов для разных платформ — общая схема, разные шаги._
