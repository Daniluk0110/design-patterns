...
## 🎯 Применимость
- **Нужно использовать разные вариации одного алгоритма в одном объекте.** Поведение можно менять во время выполнения.
- **Есть много похожих классов, отличающихся только поведением.** Поведение выносится в отдельные стратегии.
- **Нельзя раскрывать детали реализации алгоритмов.** Код и зависимости изолируются внутри стратегий.
- **Логика разрослась в длинный условный оператор.** Каждая ветка становится отдельной стратегией.
...
## ⚡ В двух словах
Можно менять алгоритм на лету, подставляя нужную стратегию.

## 🧭 Когда использовать
- Есть несколько алгоритмов для одной задачи.
- Нужна возможность переключать поведение во время выполнения.
- Хочется убрать большие `if/else` или `switch`.

## 💡 Ключевые идеи
- Стратегии реализуют единый интерфейс.
...
_Сортировка: для небольших массивов один алгоритм, для больших — другой._
