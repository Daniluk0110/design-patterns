...
## ⚡ В двух словах
Сохраняем «снимок» состояния и можем откатить объект назад.

## 🧭 Когда использовать
- Нужны **undo/redo** операции.
- Важно сохранять состояние без раскрытия внутренностей объекта.
- Нужно «сохранить точку» и откатиться к ней.

## 💡 Ключевые идеи
- Снимок хранит состояние, но не нарушает инкапсуляцию.
- Хранение снимков обычно вынесено в отдельный объект.
- История может быть цепочкой/стеком.
...
_Калькулятор сохраняет состояние и откатывается._

```php
<?php

$calculator = new Calculator();
$calculator->setCurrent(10);
$calculator->save();

$calculator->setCurrent(100);
$calculator->undo();
```
