...
## 🎯 Применимость
- **Сложная структура данных, детали которой нужно скрыть.** Итератор предоставляет несколько простых методов перебора и защищает коллекцию от неосторожных действий.
- **Нужны разные варианты обхода одной структуры.** Код обхода выносится в отдельные итераторы, не захламляя классы коллекций.
- **Единый интерфейс для разных коллекций.** Итераторы разных типов структур взаимозаменяемы.
...
## ⚡ В двух словах
Даёт единый способ обхода коллекции без раскрытия её внутреннего устройства.

## 🧭 Когда использовать
- Нужно **скрыть детали хранения** коллекции.
- Нужны **разные способы обхода** одной структуры.
- Важно предоставить единый интерфейс перебора.

## 💡 Ключевые идеи
- Итератор знает, как обходить коллекцию.
- Коллекция не раскрывает внутреннюю структуру.
- Можно менять алгоритм обхода без изменения коллекции.

## 🧪 Пример использования в PHP

### 1) Определите интерфейс `Iterator` для перебора элементов
```php
<?php
...
### 2) Реализуйте класс `ConcreteIterator`
```php
<?php
...
### 3) Создайте класс `Aggregate`, возвращающий итератор
...
_Проигрывание радиостанций по очереди._

```php
<?php

$stations = new RadioStationCollection();
$stations->add(new RadioStation(89.0));
$stations->add(new RadioStation(101.1));
