...
## ⚡ В двух словах
Прокси — это «дверь» перед объектом, где можно добавить проверки, логирование или кеш.

## 🧩 Пример из Design Patterns for Humans
_Дверь с проверкой доступа._

```php
<?php

$door = new LabDoor();
$secured = new SecuredDoor($door);

$secured->open('invalid'); // Door is locked
$secured->open('$ecr@t');  // Door opened
```
